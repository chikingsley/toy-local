...
final class ToyLocalSettingsMigrationTests: XCTestCase {
	func testV1FixtureMigratesToCurrentDefaults() throws {
		let data = try loadFixture(named: "v1")
		let decoded = try JSONDecoder().decode(ToyLocalSettings.self, from: data)
		...
		XCTAssertEqual(decoded.soundEffectsVolume, ToyLocalSettings.baseSoundEffectsVolume)
	...
	private func loadFixture(named name: String) throws -> Data {
		guard let url = Bundle.module.url(
			forResource: name,
			withExtension: "json",
			subdirectory: "Fixtures/ToyLocalSettings"
		) else {
