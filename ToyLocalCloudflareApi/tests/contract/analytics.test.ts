...
describe("Analytics Engine query builder", () => {
  it("builds a bounded usage summary query with escaped account keys", () => {
    const { dataset, query } = usageAnalyticsQuery(
      { TOY_LOCAL_USAGE_DATASET: "toy_local_usage" } as Env,
      {
    ...
    expect(dataset).toBe("toy_local_usage");
    expect(query).toContain("FROM toy_local_usage");
  ...
  it("rejects unsafe dataset identifiers", () => {
    expect(() =>
      usageAnalyticsQuery({ TOY_LOCAL_USAGE_DATASET: "bad-name;" } as Env)
