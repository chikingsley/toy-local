...
Toy Local Cloud is the API boundary for cloud ASR models, realtime ASR models, language models, licensing, request logging, usage tracking, and provider routing.

The cloud API uses direct HTTP for request/response provider calls and WebSockets for realtime provider sessions. The app talks to Toy Local Cloud. Toy Local Cloud talks to Mistral, Deepgram, ElevenLabs, and later providers.

Toy Local exposes product routes under the Toy Local domain. Provider selection happens through model IDs, credentials, and routing configuration behind those routes.
...
Toy Local implementation:

- `POST /v1/text-transforms` routes to a configured language model.
- `POST /v1/transcriptions` can route batch jobs to Mistral ASR.
- `GET /v1/realtime` upgrades to a Toy Local WebSocket, then the Durable Object opens the Mistral realtime session.
...
- Realtime ASR uses a Toy Local WebSocket through the Durable Object if enabled.
- Deepgram callback mode maps to Toy Local jobs: Deepgram returns a request ID quickly, later POSTs the result to Toy Local, and Toy Local stores the canonical job result.
...
- Realtime ASR uses the Toy Local WebSocket and Durable Object proxy.
- Webhook-based transcription results map to Toy Local jobs.

## API Product Shape

Toy Local Cloud has three transcription/language-model paths.

### 1. Dictation and Language-Model Request/Response

Request/response means one Toy Local HTTP request returns the result directly.
...
1. App asks Toy Local Cloud for an upload reservation.
2. Worker creates D1 upload metadata and returns a Toy Local upload URL.
3. App uploads audio/video through the Toy Local upload route, which stores the source media in R2.
...
Live dictation uses a Toy Local WebSocket.
...
1. App opens a WebSocket to Toy Local Cloud.
2. Worker routes the session to a Durable Object.
3. Durable Object accepts the Toy Local realtime session and normalizes Toy Local control/audio events.
...
The Worker upload route enforces the Cloudflare request body limit for the account plan and Toy Local product limits.

D1 stores upload metadata. Raw media bytes live in R2.

## BYOK and Managed Cloud

Toy Local supports three provider credential modes.

```text
managed:
  provider key/account owned by Toy Local Cloud
  user never sees upstream provider credential

local_byok:
  provider key stored in app Keychain
  app sends key to Toy Local Cloud only for a request/session
  Toy Local Cloud treats the provider key as request/session scoped

server_byok:
  provider key stored encrypted by Toy Local Cloud
  Toy Local Cloud selects it by provider_credential_id
...
Toy Local Cloud stays in the path for BYOK so the app keeps one API contract, realtime sessions still use our Durable Object, and request logging/usage normalization stays consistent.

## Auth, Licensing, and App Credentials

Toy Local API credentials are independent of provider credentials.
...
- API request auth: `Authorization: Bearer <toy-local-credential>`.
- Server storage: hash Toy Local secrets before storing in D1.

Activation flow:

```text
1. User buys Toy Local Cloud.
2. User receives a license key by email.
3. App sends license key, machine fingerprint, and client metadata to Toy Local Cloud.
4. Server validates license status and activation limits.
5. Server creates or updates license_activation and api_key rows.
6. Server returns a Toy Local API credential or short-lived bootstrap token.
...
- Pin Toy Local Cloud public keys in the macOS app.
...
Payment and licensing are separate modules internally. Toy Local keeps its own `license_activations` and `api_keys` tables no matter which checkout provider is used.
...
| Custom Toy Local licensing | Cloudflare D1 plus email/payment webhooks | Full control over activations and API credentials; more support and fraud surface. |

Starting choice:

- Use Lemon Squeezy or Polar for checkout and webhook flow.
- Keep Toy Local as the license/API authority.
- Add Keygen only if the in-house activation/device-limit implementation becomes too expensive or brittle.

## Model Registry

Toy Local public model IDs are product contracts. Provider model IDs are implementation details behind the API.
...
OpenAI-style model list endpoints are curated public availability surfaces. Toy Local can expose an internal model registry route from the code registry plus active D1 route overrides when the app needs runtime discovery.
...
- Licensing depth: Toy Local activation tables first vs Keygen earlier.
