...
private let hotKeyLogger = TimberVoxLog.hotKey
...
public struct HotKeyProcessor {
  ...
  public static let doubleTapThreshold: TimeInterval = TimberVoxCoreConstants.doubleTapWindow
  ...
  public static let pressAndHoldCancelThreshold: TimeInterval = TimberVoxCoreConstants.pressAndHoldCancelWindow
  ...
  public init(hotkey: HotKey, useDoubleTapOnly: Bool = false, minimumKeyTime: TimeInterval = TimberVoxCoreConstants.defaultMinimumKeyTime) {
    self.hotkey = hotkey
  ...
  public mutating func processMouseClick() -> Output? {
    ...
    case .idle:
      return nil
    case .pressAndHold(let startTime):
...
extension HotKeyProcessor {
  ...
  private mutating func handleNonmatchingChord(_ e: KeyEvent) -> Output? {
    ...
    case .pressAndHold(let startTime):
      ...
      let elapsed = now().timeIntervalSince(startTime)
      if hotkey.key == nil {
        let minimum = max(minimumKeyTime, RecordingDecisionEngine.modifierOnlyMinimumDuration)
        if elapsed < minimum {
          isDirty = true
          resetToIdle()
          return .discard
  ...
  private func isReleaseForActiveHotkey(_ e: KeyEvent) -> Bool {
    if hotkey.key != nil {
      ...
      return keyReleased && modifiersAreSubset
    }
