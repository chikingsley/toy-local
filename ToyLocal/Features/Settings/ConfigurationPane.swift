...
struct ConfigurationPane: View {
  ...
  private var theme: ConfigurationThemeChoice {
    switch store.toyLocalSettings.appearancePreference {
    case .light: .light
    case .dark: .dark
          ...
          ConfigurationVisualChoiceGroup {
            ForEach(ConfigurationThemeChoice.allCases) { option in
              ConfigurationVisualChoice(label: option.label, selected: theme == option) {
                store.toyLocalSettings.appearancePreference = option.preference
              } preview: {
    ...
    TLSection(title: "Application") {
      TLSettingsCard {
        ...
        ConfigurationMenuRow(
          ...
          selection: $store.toyLocalSettings.recordingRetention
        )
    ...
    TLSection(title: "Text Input") {
      TLSettingsCard {
        ...
        ConfigurationMenuRow(
          ...
          selection: $store.toyLocalSettings.clipboardRestoreBehavior
        )
        TLSettingsToggleRow(
          icon: "keyboard",
          ...
          showsAI: true,
          isOn: Binding(
            get: { !store.toyLocalSettings.useClipboardPaste },
            set: { store.toyLocalSettings.useClipboardPaste = !$0 }
          )
        )
