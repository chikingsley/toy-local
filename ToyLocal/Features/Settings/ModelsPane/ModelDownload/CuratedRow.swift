...
struct CuratedRow: View {
  ...
  var isSelected: Bool {
    let selected = store.selectedIdentifier(for: model)
  ...
  private var downloadProgressLabel: String {
    store.downloadProgress.formatted(.percent.precision(.fractionLength(0)))
  }
            ...
            ZStack {
              if store.isDownloading, store.downloadingModelName == model.internalName {
                ProgressView(value: store.downloadProgress)
                  ...
                  .help("Downloading... \(downloadProgressLabel)")
              } else if model.isDownloaded {
                Image(systemName: "checkmark.circle.fill")
                  .foregroundStyle(.green)
                  .frame(width: 24, height: 24)
                  .help("Downloaded")
              } else if model.isRuntimeSelectable {
                Button {
                  store.selectModel(model.internalName)
                  store.downloadModel(model.internalName)
                } label: {
                ...
                Image(systemName: "clock.badge.questionmark")
                  .foregroundStyle(.secondary)
                  .frame(width: 24, height: 24)
                  .help("Support asset wiring pending")
              }
            }
          }
        }
    ...
    .contextMenu {
      ...
      if model.isDownloaded {
        ...
        Button("Delete", role: .destructive) {
          store.selectModel(model.internalName)
          store.deleteModel(model.internalName)
        }
      }
    }
  }
