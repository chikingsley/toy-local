...
extension ModesPane {
  private func syncDefaultModeFromSettings() {
    ...
    modes[index] = ModeDraft.defaultMode(
      from: store.toyLocalSettings,
      languageName: languageName(forCode: store.toyLocalSettings.outputLanguage)
    )
  }

  private func applyDefaultMode(_ mode: ModeDraft) {
    store.toyLocalSettings.textTransformMode = mode.preset.transformMode
    store.toyLocalSettings.outputLanguage = languageCode(forName: mode.language)
    store.toyLocalSettings.selectedModel = mode.voiceModel.id
    store.toyLocalSettings.textTransformModel = mode.languageModel.id
    store.toyLocalSettings.recordingAudioBehavior = ModeDraft.audioBehavior(forLabel: mode.playbackBehavior)
    store.toyLocalSettings.recordingInputMode = mode.recordSystemAudio ? .systemAudio : .microphone
    store.toyLocalSettings.autoPasteResult = mode.autoPaste != "Off"
  }
