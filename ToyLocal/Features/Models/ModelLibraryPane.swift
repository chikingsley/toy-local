...
struct ModelLibraryPane: View {
  ...
  private func isSelected(_ row: ModelLibraryRow) -> Bool {
    ...
    case .localDictation, .cloudDictation:
      store.toyLocalSettings.selectedModel == row.entry.id
    case .streamingPreview:
      store.toyLocalSettings.alwaysOnStreamingModel == row.entry.id
    case .cloudText:
      store.toyLocalSettings.textTransformModel == row.entry.id
    case .supportAssets:
      false
    }
  }
