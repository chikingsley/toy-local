...
private let contextCaptureLogger = ToyLocalLog.transcription
...
final class DictationContextCaptureClientLive {
  ...
  private static func attachmentDirectory() -> URL? {
    do {
      let directory = try URL.toyLocalApplicationSupport.appending(component: "ContextAttachments", directoryHint: .isDirectory)
  ...
  private func syncClipboardMonitorState(for settings: ToyLocalSettings) {
    let shouldMonitor =
...
final class DictationContextCaptureSession {
  ...
  private let settings: ToyLocalSettings
  private let clipboardMonitor: DictationClipboardMonitor
  private let attachmentDirectory: URL?
  ...
  private static func currentContext(settings: ToyLocalSettings, capturedAt: Date) -> DictationContext {
  ...
  private static func systemContext(settings: ToyLocalSettings, capturedAt: Date) -> SystemContext {
    SystemContext(
  ...
  private static func vocabulary(from settings: ToyLocalSettings) -> [String] {
