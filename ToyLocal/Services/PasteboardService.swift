...
struct PasteboardClientLive {
  ...
  func paste(text: String) async -> Bool {
    let mode = toyLocalSettings.useClipboardPaste ? "clipboard" : "accessibility"
    pasteboardLogger.info(
      "Paste requested mode=\(mode) chars=\(text.count) copyToClipboard=\(toyLocalSettings.copyToClipboard)"
    )

    if toyLocalSettings.useClipboardPaste {
  ...
  func pasteWithClipboard(_ text: String) async -> Bool {
    ...
    if !pasteSucceeded && !toyLocalSettings.copyToClipboard {
