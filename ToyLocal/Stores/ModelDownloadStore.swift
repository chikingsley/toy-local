...
final class ModelDownloadStore {
  ...
  func selectModel(_ model: String) {
    ...
    if isStreaming {
      toyLocalSettings.alwaysOnStreamingModel = resolved
    } else {
      toyLocalSettings.selectedModel = resolved
    }
  ...
  func downloadSelectedModel() {
    downloadModel(toyLocalSettings.selectedModel)
  }
  ...
  private func handleModelsLoaded(recommended: String, available: [ModelInfo]) {
    ...
    if toyLocalSettings.selectedModel.contains("*") || toyLocalSettings.selectedModel.contains("?") {
      if let resolved = resolvePattern(toyLocalSettings.selectedModel, from: available) {
        toyLocalSettings.selectedModel = resolved
      }
    }
    if !availablePlus.contains(where: { $0.name == toyLocalSettings.selectedModel }) {
      let replacement = recommendedModel.isEmpty ? FluidAudioModels.parakeetTdtV3.id : recommendedModel
      toyLocalSettings.selectedModel = replacement
    }
    ...
    if !anyModelDownloaded && !toyLocalSettings.hasCompletedModelBootstrap {
      let preferred = recommendedModel.isEmpty ? toyLocalSettings.selectedModel : recommendedModel
      if !preferred.isEmpty {
        toyLocalSettings.selectedModel = preferred
        updateBootstrapState()
      }
    }
  }
  ...
  private func updateBootstrapState() {
    let model = toyLocalSettings.selectedModel
    guard !model.isEmpty else { return }
