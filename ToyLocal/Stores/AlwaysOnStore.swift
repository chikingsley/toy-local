...
private extension AlwaysOnStore {
  ...
  func pasteResolvedBuffer(_ initialText: String) {
    ...
    if toyLocalSettings.wordRemovalsEnabled {
      text = WordRemovalApplier.apply(text, removals: toyLocalSettings.wordRemovals)
    }
    text = WordRemappingApplier.apply(text, remappings: toyLocalSettings.wordRemappings)
