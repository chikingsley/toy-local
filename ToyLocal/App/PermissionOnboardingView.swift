...
struct PermissionOnboardingView: View {
  ...
  private var welcomeStep: some View {
    VStack(spacing: 22) {
      ...
      VStack(spacing: 8) {
        Text("Set up ToyLocal")
          .font(.largeTitle.weight(.semibold))
          .multilineTextAlignment(.center)
        Text("Grant the permissions ToyLocal needs for dictation, context, and typing.")
          .font(.body)
  ...
  private var permissionsStep: some View {
    VStack(alignment: .leading, spacing: 18) {
      ...
      VStack(spacing: 10) {
        permissionRow(
          title: "Microphone",
          message: "Records your voice so ToyLocal can transcribe dictation locally.",
      ...
      if manager.status(for: .accessibility) != .granted {
        Label(
          "After enabling Accessibility in System Settings, return to ToyLocal. This window will update automatically.",
