...
enum DebugStateReporter {
  static var stateURL: URL {
    get throws {
      try URL.toyLocalApplicationSupport.appending(component: "debug-state.json")
    }
  }
  ...
  static func writeSnapshot(
    ...
    do {
      ...
      ToyLocalLog.app.info("Wrote debug state to \(destinationURL.path)")
      return snapshot
    } catch {
      ToyLocalLog.app.error("Failed to write debug state: \(error.localizedDescription)")
      return nil
    }
  }
