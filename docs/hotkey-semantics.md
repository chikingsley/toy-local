# TimberVox HotKey Semantics

## Overview

TimberVox uses a **threshold-based recording system** that behaves differently depending on whether your hotkey is **modifier-only** (e.g., Option) or **a regular hotkey** (e.g., Cmd+A).
...
**When does TimberVox intercept (block) key events from reaching other apps?**
...
Result: TimberVox discards recording silently
        macOS sees Option+A
...
TimberVox: Silent discard, no transcription
```
...
**Why:** After 0.3s, TimberVox assumes you're intentionally recording and ignores other input (except ESC).
...
- **Core Logic**: `TimberVoxCore/Sources/TimberVoxCore/Logic/HotKeyProcessor.swift`
- **Recording Decision**: `TimberVoxCore/Sources/TimberVoxCore/Logic/RecordingDecision.swift`
- **Feature Integration**: `TimberVox/Features/Transcription/TranscriptionFeature.swift`
- **Tests**: `TimberVoxCore/Tests/TimberVoxCoreTests/HotKeyProcessorTests.swift`
