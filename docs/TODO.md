...
- [ ] MAS build variant: second build configuration with sandbox ON, a MAS entitlements file (app-sandbox, audio-input, network.client, user-selected files; NO apple-events, NO Sparkle mach-lookup exceptions), Sparkle and MediaRemote and the AppleScript pause path compiled out, hardened-runtime/signing per App Store requirements. [A for build; C for Chi's sandboxed feature pass]
...
- [ ] App Store Connect: app record for the MAS bundle id, Paid Apps agreement + banking/tax (required before IAP), IAP products mirroring the RC offering, privacy nutrition labels (mic, transcripts, cloud endpoints), screenshots, category, review notes explaining Accessibility/Input Monitoring usage, TestFlight build before submission. [C: Chi owns the account-level steps]
