...
private let mediaLogger = TimberVoxLog.media
...
func pauseAllMediaApplications() -> [String] {
  ...
  let appleScript = NSAppleScript(source: pauseMediaApplicationsScript())
  var error: NSDictionary?
  guard let resultDescriptor = appleScript?.executeAndReturnError(&error) else {
    handlePauseMediaApplicationsError(error)
    return []
  }

  let pausedPlayers = pausedPlayers(from: resultDescriptor)
  mediaLogger.notice("Paused media players: \(pausedPlayers.joined(separator: ", "))")

  return pausedPlayers
}

private func pauseMediaApplicationsScript() -> String {
  var scriptParts: [String] = ["set pausedPlayers to {}"]

  for (appName, _) in installedMediaPlayers {
    scriptParts.append(pauseMediaApplicationScript(appName: appName))
  }
...
private func pauseMediaApplicationScript(appName: String) -> String {
  if appName == "VLC" {
    return """
      try
...
private func handlePauseMediaApplicationsError(_ error: NSDictionary?) {
...
private func pausedPlayers(from resultDescriptor: NSAppleEventDescriptor) -> [String] {
