...
private let pasteboardLogger = TimberVoxLog.pasteboard

@MainActor
struct PasteboardClientLive {
  let settingsManager: SettingsManager

  private var timberVoxSettings: TimberVoxSettings { settingsManager.settings }
  private let clipboardRestoreDelay: Duration = .milliseconds(1_200)

  private var shouldRestoreClipboard: Bool {
    switch timberVoxSettings.clipboardRestoreBehavior {
    case .defaultBehavior: !timberVoxSettings.copyToClipboard
    case .restore: true
    case .bypass: false
    }
  }
  ...
  func paste(text: String) async -> Bool {
    let mode = timberVoxSettings.useClipboardPaste ? "clipboard" : "accessibility"
    pasteboardLogger.info(
      "Paste requested mode=\(mode) chars=\(text.count) copyToClipboard=\(timberVoxSettings.copyToClipboard)"
    )

    if timberVoxSettings.useClipboardPaste {
  ...
  func pasteWithClipboard(_ text: String) async -> Bool {
    ...
    if !pasteSucceeded && !timberVoxSettings.copyToClipboard {
