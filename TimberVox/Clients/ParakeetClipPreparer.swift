...
enum ParakeetClipPreparer {
  ...
  static func ensureMinimumDuration(
    ...
    logger: os.Logger = TimberVoxLog.parakeet
  ) throws -> ParakeetClipPreparationResult {
    let audioFile = try AVAudioFile(forReading: url)
    let format = audioFile.processingFormat
    let duration = Double(audioFile.length) / format.sampleRate
    logger.debug("\(clipSummary(url: url, duration: duration, format: format), privacy: .public)")
    ...
    logger.notice("\(paddedSummary(inputURL: url, outputURL: paddedURL, duration: duration, minimumDuration: minimumDuration), privacy: .public)")
  ...
  private static func clipSummary(url: URL, duration: TimeInterval, format: AVAudioFormat) -> String {
    let durationDescription = String(format: "%.3f", duration)
    let sampleRateDescription = String(format: "%.0f", format.sampleRate)
    return [
  ...
  private static func paddedSummary(
    inputURL: URL,
    outputURL: URL,
    ...
    let durationDescription = String(format: "%.3f", duration)
    let minimumDurationDescription = String(format: "%.3f", minimumDuration)
    return [
