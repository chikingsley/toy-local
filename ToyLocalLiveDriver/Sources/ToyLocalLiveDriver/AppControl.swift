...
struct AppControl {
  ...
  var stateURL: URL {
    URL.toyLocalApplicationSupport.appendingPathComponent("debug-state.json")
  }
  ...
  func launch() throws {
    ...
    environment["TOYLOCAL_DISABLE_SPARKLE"] = "1"
  ...
  func requestState(timeout: TimeInterval = 5) throws -> DebugStateSnapshot {
    try FileManager.default.createDirectory(
      at: URL.toyLocalApplicationSupport,
      withIntermediateDirectories: true
    )
    let previousModification = stateModificationDate()
    try openURL("toylocal-debug://state")
  ...
  private func appBundleURL() throws -> URL {
    ...
    while let url = enumerator?.nextObject() as? URL {
      guard url.lastPathComponent == "toy-local.app",
    ...
    guard let app = candidates.max(by: { $0.path < $1.path }) else {
      throw DriverError(
        "No \(target.configuration) toy-local.app found in DerivedData. Build the app first."
      )
    }
    return app
  }
