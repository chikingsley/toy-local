...
final class AppStore {
	...
	nonisolated(unsafe) private var pasteLastTranscriptTask: Task<Void, Never>?
	nonisolated(unsafe) private var permissionMonitorTask: Task<Void, Never>?
	nonisolated(unsafe) private var modelReadinessTask: Task<Void, Never>?
	...
	private func startPermissionMonitoring() {
		...
		permissionMonitorTask = Task { [weak self] in
			...
			for await activation in await self.permissions.observeAppActivation() {
