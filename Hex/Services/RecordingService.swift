...
/// Simulates a media key press (the Play/Pause key)
...
actor RecordingClientLive {
	...
	private func ensureInputDeviceUnmuted(settings: HexSettings) {
	...
	func startRecording() async {
		// Read settings snapshot once at the start
		let hexSettings = await settingsManager.settings

		// Check and fix device-level mute before recording
		ensureInputDeviceUnmuted(settings: hexSettings)
