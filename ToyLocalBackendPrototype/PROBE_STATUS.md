...
- `swift build` passes for `ToyLocalBackendPrototype`.
...
- `vad --audio ../ToyLocalCloudflareApi/tests/fixtures/audio/asr-smoke.wav` works with Silero VAD.
...
- `keyword --terms "Toy Local,FluidAudio,Parakeet" --audio ... --model ctc110m` works and records detection timings plus the CTC log-probability matrix shape.
- `keyword --terms "Toy Local,FluidAudio,Parakeet" --audio ... --model ctc06b` works after a long uninterrupted download/compile. The public CTC loader exposes little progress for the first large bundle, so the earlier interruption was premature.
